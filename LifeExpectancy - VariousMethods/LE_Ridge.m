%% %% Life expectancy dataset with Ridge regression
clear all
runtime = zeros(1,50); 
for p = 1:50
    clearvars('-except','p','runtime','optlambda')
    tic
...
k = find(mse==min(mse));
optlambda(p) = lambda(k)

% Reestimating w using full data set
w = ridge_regression(trainval_X,trainval_y,optlambda(p));
